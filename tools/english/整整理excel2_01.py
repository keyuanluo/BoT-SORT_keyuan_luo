...
#         print("\n[Warning] 以下行 BBox 不符合 (x2>=x1, y2>=y1) 要求：")
...
#     # 读取 Excel 文件
#     input_excel = '/media/robert/4TB-SSD/watchped_dataset - 副本/Excel_Box_video_number/video_0021/detections_过渡.xlsx'  # 替换为您的 Excel 文件名
#     df = pd.read_excel(input_excel)
#
#     # 若要只处理特定 track_id，可以像这样指定
#     # track_id_list = [1, 27, 31, 38, 58]
...
#
#     # 设置输出文件路径，与输入文件在相同的目录下
#     output_excel = os.path.join(input_dir, 'detections_最终.xlsx')  # 输出文件名
#
#     # 将填充后的数据保存到新的 Excel 文件
#     filled_df.to_excel(output_excel, index=False)
#
#     print(f'\n填充后的数据已保存到 {output_excel}')
...
def fill_missing_frames(df, track_id_list=None):
    ...
    # =============== 去重逻辑：处理“同一 track_id + 同一 image_name”重复行 ===============
    # 下面给出两种常用的方式，任选其一即可

    # 方式 A: 直接 drop_duplicates, 保留第一行
    #   如果同一个 (track_id, image_name) 出现多次，只保留先出现那行
    filled_df = filled_df.drop_duplicates(subset=['track_id', 'image_name'], keep='first')

    # 方式 B: 若想要保留分数最高的那行(假设score列越大越好)，可以用 groupby + idxmax
    # 注意：需保证score不是NaN
    # filled_df = (
...
if __name__ == "__main__":
    # 读取 Excel 文件
    input_excel = '/media/robert/4TB-SSD/watchped_dataset - 副本/Excel_Box_video_number/video_0016/detections_过渡.xlsx'  # 替换为您的 Excel 文件名
    df = pd.read_excel(input_excel)

    # -------- 关键：在处理之前，把 track_id 全部赋值为 1 --------
    df['track_id'] = 1
