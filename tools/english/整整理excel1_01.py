...
def fill_missing_frames(df, track_id_list=None):
    ...
    if not invalid_rows.empty:
        print("[Warning] 以下行的 x2 < x1 或 y2 < y1：")
        # 按需打印你关注的列
...
# 示例用法：
# 读取 Excel 文件
input_excel = '/media/robert/4TB-SSD/watchped_dataset - 副本/Excel_Box_video_number/video_0016/detections.xlsx'  # 替换为您的 Excel 文件名
df = pd.read_excel(input_excel)

track_id_list = [1,2]# 指定要处理的 track_id 列表
...
output_excel = os.path.join(input_dir, 'detections_过渡.xlsx')  # 输出文件名
